...
public final class LogCatBuilder {
	...
	public static void WriteErrorToLog(Context p_context, String p_tag,
		...
		if (p_exception == null) {
			throw new RuntimeException(
					"Avec un niveau de log Erreur, il est obligatoire de passer une exception en parametre");

		} else {
			// si egal null, marque "inconnu" sinon, marque le message
			String s = (p_exception.getMessage() == null ? "Exception inconnue"
	 ...
	 * @param p_context - Context
	 * @param p_niveauLog - {@link EnNiveauLog} - le niveau d'information du
	 *            message (info, debug...)
	 * @param p_tag - String - le tag de la classe java
	 * @param p_resId - int - le lien vers le fichier {@link string}
	 * @throws RuntimeException - si on utilise un niveau d'info
	 *             {@link EnNiveauLog.ERROR}
	 */
	public static void WriteInfoToLog(Context p_context,
			EnNiveauLog p_niveauLog, String p_tag, int p_resId) {
		switch (p_niveauLog) {
			case VERBOSE:
			...
			case ERROR:
				throw new RuntimeException(
						"Cette methode n'est pas faites pour la gestion des erreurs, utilisez WriteErrorToLog");
		}
	}
